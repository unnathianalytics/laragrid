...
final class SelectFilter extends Filter
{
    ...
    /**
     * Per-request memo of the resolved options map — ->toArray() (config) and ->accepts()
     * (restored-value validation) both need it, and the closure is usually a DB lookup.
     *
    ...
    /**
     * A restored value is legal only while it is still one of the CURRENTLY resolved options —
     * which are looked up inside the host's bound tenant, so an id carried over from another
     * company (or pointing at a deleted record) drops instead of emptying the list.
     *
     * A filter declaring no options at all has nothing to validate against and accepts anything.
     */
    ...
    /**
     * The resolved (and memoised) {value => label} option map.
     *
     * @return array<int|string, string>
     */
    private function optionMap(): array
    {
        return $this->resolvedOptions ??= $this->resolveOptions($this->options);
    }
}
